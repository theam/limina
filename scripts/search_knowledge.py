...
def parse_yaml_frontmatter(text: str) -> dict[str, str]:
    """Parse YAML frontmatter fields as simple key: value pairs."""
    fm: dict[str, str] = {}
...
def score_card(query_terms: list[str], text_lower: str, yaml_fm: dict[str, str]) -> float:
    """Score a card by term frequency + section weighting + description bonus."""
    score = 0.0
    description_lower = yaml_fm.get("description", "").lower()
    ...
    for term in query_terms:
        ...
        if term in first_line:
            score += 3
        # Bonus: term in description field (retrieval-optimized natural language)
        if description_lower and term in description_lower:
            score += 2
...
def search(shared_dir: Path, query: str, top_n: int) -> list[dict]:
    ...
    for path in sorted(cards_dir.glob("K*.md")):
        ...
        yaml_fm = parse_yaml_frontmatter(text)

        # Skip retired/superseded cards
        status = yaml_fm.get("status", "ACTIVE").upper()
        if status in ("RETIRED", "SUPERSEDED"):
            continue

        score = score_card(query_terms, text_lower, yaml_fm)
        if score == 0:
            continue
        ...
        # Prefer YAML frontmatter fields, fall back to blockquote metadata
        confidence = yaml_fm.get("confidence", meta.get("Confidence", "—"))
        domain = yaml_fm.get("domain", meta.get("Domain", "—"))
        source = yaml_fm.get("source_mission", meta.get("Source mission", "—"))
